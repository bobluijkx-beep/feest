...
import { prisma, listActiveBundlesForEvent } from "@lions/core";
...
export default async function ProductsPage({ params }: { params: Promise<{ eventSlug: string }> }) {
  ...
  const [products, bundles] = await Promise.all([
    ...
    listActiveBundlesForEvent(event.id),
  ]);
  ...
  return (
    <main className="mx-auto max-w-2xl px-4 py-8 md:max-w-4xl lg:max-w-6xl">
      ...
      {bundles.length > 0 && (
        <div className="mt-6">
          <h2 className="mb-3 font-heading text-base font-medium">Combi&apos;s</h2>
          <BundleGrid bundles={bundles} eventSlug={eventSlug} />
        </div>
      )}
